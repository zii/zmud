...
| `#loop N` | 循环 N 次后停止 | `#loop 5`（循环5次） |
| `#loop $var` | 循环 $var 次 | `#loop $nl`（内力值次） |
...
**跳转方式**：
- `#if $hp<30 3` — 跳转到第 3 条指令
- `#if $hp>100 +2` — 相对跳转（前进 2 条）
- `#if $hp<30 -1` — 相对跳转（后退 1 条）
...
通过 `/alias` 定义命令缩写，支持位置参数 `$A1`~`$A9` 和完整参数 `$*`。
...
# $* 传递别名后所有文本（不拆分空格）
/alias j ask $* about jianxi
j jian ke   # 展开为：ask jian ke about jianxi
...
| `/ask <问题>` | 询问AI任何问题 |
| `/hint` | 获取基于上下文的AI建议 |
| `/r` | 查看服务器最近一条原始文本（调试翻译用） |
| `/r N` | 查看倒数第 N 条原始文本 |
| `/back <路径>` | 反解方向列表为路径（`/back n;s;e`） |
| `/debug` | 切换调试模式，查看变量和执行流程 |
| `/quit` | 退出游戏 |
| `/alias` | 查看别名列表 |
| `/alias key none` | 删除别名 |
...
```
zmud              # 交互选择服务器
zmud 3            # 直接连接服务器列表中的第 3 个
```

## 命令历史

历史记录持久化存储在 LMDB 中（`~/.zmud/<host>:<port>.db`），退出或崩溃不丢失。
每条命令即时持久化，Tab 补全按最近使用排序。
