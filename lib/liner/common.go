...
// SetKeyBinding 设置自定义按键绑定。key 格式如 "ctrl-a", "ctrl-r", "ctrl-p", "ctrl-t",
// "f1", "f2", "up", "down", "home", "end", "alt-d", "alt-b" 等。
// handler 是一个函数，接收 *State 参数，可以在 handler 中访问和修改当前编辑的行内容。
func (s *State) SetKeyBinding(key string, handler func(*State)) {
	if s.keyBindings == nil {
		s.keyBindings = make(map[string]func(*State))
	}
	s.keyBindings[key] = handler
}

// RemoveKeyBinding 移除指定的自定义按键绑定。
func (s *State) RemoveKeyBinding(key string) {
	if s.keyBindings != nil {
		delete(s.keyBindings, key)
	}
}

// ClearKeyBindings 清除所有自定义按键绑定。
func (s *State) ClearKeyBindings() {
	s.keyBindings = nil
}
