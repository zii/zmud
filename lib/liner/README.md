...
You can set custom key bindings to override the default behavior. The supported
keys are: "ctrl-a", "ctrl-b", "ctrl-e", "ctrl-f", "ctrl-l", "ctrl-n", "ctrl-p", "ctrl-r".
...
// Override Ctrl+R for custom behavior
line.SetKeyBinding("ctrl-r", func(s *liner.State) {
    fmt.Println("\nCustom reverse search!")
    // s.currentLine and s.currentPos are available to access/modify the line
})

// Bind Ctrl+P to a custom function
line.SetKeyBinding("ctrl-p", func(s *liner.State) {
    fmt.Println("\nCustom previous history!")
})
```

To remove a custom binding:

```go
line.RemoveKeyBinding("ctrl-r")
```

To clear all custom bindings:

```go
line.ClearKeyBindings()
```
